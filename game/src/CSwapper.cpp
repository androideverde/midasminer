...
bool CSwapper::DoSwap(SBoardCoords tileCoords_1, SBoardCoords tileCoords_2)
{
	...
	if (!(mMatcher.IsMatchInTile(tileCoords_1) || mMatcher.IsMatchInTile(tileCoords_2)))
	{
		...
		TriggerSwapOutAnimations(tileCoords_2, tileCoords_1);
...
void CSwapper::TriggerSwapInAnimations(SBoardCoords tile_1, SBoardCoords tile_2)
{
	...
	parallelAnims.emplace_back(std::make_unique<CMoveAnimation>(candy_1->GetPos(), candy_2->GetPos(), 300.f, candy_1));
	parallelAnims.emplace_back(std::make_unique<CMoveAnimation>(candy_2->GetPos(), candy_1->GetPos(), 300.f, candy_2));
...
void CSwapper::TriggerSwapOutAnimations(SBoardCoords tile_1, SBoardCoords tile_2)
{
	...
	std::vector<std::unique_ptr<CAnimation>> parallelAnims;
	parallelAnims.emplace_back(std::make_unique<CMoveAnimation>(candy_1->GetPos(), candy_2->GetPos(), 300.f, candy_2));
	parallelAnims.emplace_back(std::make_unique<CMoveAnimation>(candy_2->GetPos(), candy_1->GetPos(), 300.f, candy_1));
	mAnimationQueue.AddAnimation(std::make_unique<CParallelAnimation>(std::move(parallelAnims)));
}
